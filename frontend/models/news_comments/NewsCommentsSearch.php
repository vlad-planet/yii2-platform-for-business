...
class NewsCommentsSearch extends NewsComments
{
    ...
    public function search(array $params): ActiveDataProvider
    {
        ...
        if (true === $this->validate()) {
            $query->andFilterWhere(['ilike', static::ATTR_ID, $this->id]);
            $query->andFilterWhere(['ilike', static::ATTR_NEWS_ID, $this->news_id]);
            $query->andFilterWhere(['ilike', static::ATTR_USER_ID, $this->user_id]);
            $query->andFilterWhere(['ilike', static::ATTR_PARENT_ID, $this->parent_id]);
    ...
    /**
     * Модель данных соотвествующих ID news
     *
     * @param string $id
     * @return NewsCommentsSearchResult
     *
     * @author Vladislav Bashlykov
     */
    public function searchByNewsID(string $id): NewsCommentsSearchResult
    {
        $query = NewsComments::find()
        ...
        return (new NewsCommentsSearchResult($query,null));
    }
}